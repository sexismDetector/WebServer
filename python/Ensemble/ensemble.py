...
from python.SVM import TrainSVM
...
vclf1 = vclf1.fit(X_df_train, Y_df_train.loc[:,"label"])

vclf2 = VotingClassifier(estimators=[('rbf', rbf), ('lnr', lnr)], voting='soft')
vclf2 = vclf2.fit(X_df_train, Y_df_train.loc[:,"label"])
...
vclf1_score = vclf1.score(X_df_test,Y_df_test.loc[:,"label"])
vclf2_score = vclf2.score(X_df_test,Y_df_test.loc[:,"label"])
